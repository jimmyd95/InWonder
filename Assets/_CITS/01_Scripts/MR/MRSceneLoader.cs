...
public class MRSceneLoader : OVRSceneModelLoader
{
...
#else
        Debug.LogError($"[{nameof(MRSceneLoader)}]: No Scene data found. Please run Room Setup on your Quest and try again.");
    ...
    IEnumerator AwaitSceneModel()
    {
        ...
        var preamble = $"[{nameof(MRSceneLoader)}]: {nameof(SceneManager.LoadSceneModel)}";
        var attemptsRemaining = attemptCount;
        while (attemptsRemaining > 0)
        {
            if (SceneManager.LoadSceneModel())
            {
                Debug.Log($"[{nameof(MRSceneLoader)}]: Loading scene model...");
                yield break;
            }
    ...
    IEnumerator AwaitSceneCaptureCompletion()
    {
        ...
        Debug.Log($"[{nameof(MRSceneLoader)}]: Requesting Scene Capture.");
        if (SceneManager.RequestSceneCapture())
        {
            ...
            Debug.LogError($"[{nameof(MRSceneLoader)}]: Did not get a response following the completion of Scene Capture.");
        }
        else
        {
            ...
            // {
                ...
                // In WorldBeyond script, you needed to use Scene API to load the surroundings for Q2, but not in Quest 3
            // }

            Debug.LogError($"[{nameof(MRSceneLoader)}]: {nameof(SceneManager.RequestSceneCapture)} returned false.");
        }
    ...
    protected override void OnSceneCaptureReturnedWithoutError()
    {
        ...
        Debug.Log($"[{nameof(MRSceneLoader)}]: Scene Capture completed successfully.");
    ...
    {
        sceneCaptureComplete = true;
        Debug.LogError($"[{nameof(MRSceneLoader)}]: Scene Capture failed with an unexpected error.");
